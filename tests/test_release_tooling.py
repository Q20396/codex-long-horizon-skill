            ...
            if argv[:2] == ["plugin", "list"]:
                ...
                if "--json" in argv:
                    if scenario == "plugin_list_available_only":
                        print(json.dumps({"installed": [], "available": [{"name": plugin_name, "marketplaceName": marketplace_name, "version": "0.2.1"}]}))
                        raise SystemExit(0)
                    if scenario == "plugin_list_wrong_version":
                        ...
                        raise SystemExit(0)
                    print(json.dumps({"installed": [{"name": plugin_name, "marketplaceName": marketplace_name, "version": "0.2.1", "installed": installed.exists()}]}))
                elif scenario == "plugin_list_text_substring":
                    print(f"{plugin_name}-old {marketplace_name} 0.2.1 installed")
                else:
                    print(f"{plugin_name} {marketplace_name} 0.2.1 installed")
                raise SystemExit(0)
...
class ReleaseReadinessTests(unittest.TestCase):
    ...
    def run_readiness(
        ...
        *args: str,
        ...
        return subprocess.run(
            [sys.executable, "scripts/check_release_readiness.py", "--version", "0.2.1", *args],
            cwd=repo,
            env=run_env,
            text=True,
            capture_output=True,
    ...
    def release_notes(self, repo: Path) -> Path:
        return repo / "docs" / "releases" / "v0.2.1.md"
    ...
    def init_repo_with_tag(self, repo: Path) -> None:
        ...
        subprocess.run(["git", "tag", "v0.2.1"], cwd=repo, check=True, capture_output=True, text=True)
    ...
    def test_missing_dated_changelog_heading_fails(self) -> None:
        ...
        self.changelog(repo).write_text(
            self.changelog(repo).read_text(encoding="utf-8").replace("## 0.2.1 - 2026-07-15", "## 0.2.1"),
            encoding="utf-8",
        )
        result = self.run_readiness(repo, "--allow-existing-tag")
    ...
    def test_manifest_version_mismatch_fails(self) -> None:
        ...
        result = self.run_readiness(repo, "--allow-existing-tag")
        self.assert_failed_without_traceback(result, "plugin version '9.9.9' does not match '0.2.1'")
